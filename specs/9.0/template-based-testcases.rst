...
when test creates OS environment.
...
* Passing configuration of openstack environment through system environment
  variable. If use this way, we'll not have dynamic mapping confguraion and
  tests. That will make CI impact, we should add variable for configuration to
  each Jenkins jobs and maintain it every time when we add new configuration.
