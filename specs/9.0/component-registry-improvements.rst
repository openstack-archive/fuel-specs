...
them:
  ...
  * How to automatically enable some components if they are required for
    option has been chosen by user.
...
* Extend components DSL model with additional semantic:
  ...
  `not` - checks that element from list shouldn't be processed. Can be combined
  with `one_of`|`all_of`|`any_of`. Concepts are taken from [4]_
...
* Another approach is: implement `expression` logic for incompatible/requires
  relations. It should works in same way as for restrictions. Example:
