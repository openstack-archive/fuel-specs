...
The store for serialized deployment information (e.g. ConfigDB service or
Nailgun API extension [2]_) allows 3rd party applications to access it.
It also enables alternative deployment/lifecycle management solutions
to synchronize their configrations with Fuel installer. However, it
doesn't solve the problem of getting the information into the service,
since the service itself is a more or less passive store.
