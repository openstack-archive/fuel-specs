...
.. code::
  ...
  len($.nodes.where($.status = 'ready' and 'controller' in $.roles)) > 0
        ...
        $.roles and $.pending_addition = true)) > 0
...
Changes for TaskHelper should allow for a user to:
  ...
  * Identify cluster status change
    $.cluster_is('operational'|'new'|'partially_deployed')

  * Identify node number changes
    $.cluster_nodes_with_role('compute').changed()

  * Identify cluster settings changes
    $.cluster.config('nova').changed()

  * Identify node settings changes
    $.cluster_nodes_in_rack('r1').network_scheme('routes').changed()

  * Identify changes of nodes with particular labels or roles
    $.cluster_nodes_with_label('testing').changed()
