...
*   Currently they are no possibility to deploy OpenStack on nodes with
    FC HBA multipath devices, using Fuel.Otherwise, many and many Openstack
    solution are switching to use FC HBA solutions with multipath.

*   Currently they are no possibility to deploy OpenStack on disk-less nodes.
...
disks in Fuel 9.0, which means at least the following:
    ...
    * Extending fuel-agent logic and tooling(utils)

    * Extending fuel-nailgun-agent logic
...
We propose to extend Naigun to determine the multipath configuration from data
...
#. Changes in Node meta validation schema to align with recived raw
   dict-like structures from fuel-nailgun-client. Implementing interpretation
   of raw data which will output disks and multipath devices with paths and all
   nessassary information related to block devices.
...
So, we propose add logic in fuel-nailgun-agent to provide some additional
