...
Put some code in fuel-nailgun-agent to discover multipath topology. Topology
for each mulipath device should be serialized in dict and attached to multipath
device information it self. Report of mulipath device will look like this:
  ...
  .. code-block:: json
    ...
    {'disks':
     [{'name': 'dm-42',
       ...
       'multipath_topology':[
       {'disk': 'vda', 'extra': ['disk/by-id/virtio-accc7542f9ea46e7bf65']},
       {'disk': 'vdb', 'extra': ['disk/by-id/virtio-dc0a14575b534e7b86c2']}]}},...
    ]}

Underlying devices (in current sample - vda and vdb) will not be reported into
nailgun as a separate block device.

Option 2:

We remove code from fuel-nailgun-agent related to disks discovering. Only thing
which fuel-nailgun-agent will do run bunch of command and send raw output into
nailgun. In format like this:
...
Reports with raw data from the updated fuel-nailgun-agent will be handled by url
