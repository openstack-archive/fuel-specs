...
* Download the latest version of serialized deployment
  facts for the given node ID and data source
    ...
    * ``<:endpoint_uri>/environment/<:env_id>/node/<:node_id>/resource/<:datasource>/values?version=<:version>``
    ...
    * Parameters which can be passed via the URL
      ...
      * ``node_id`` is an ID of node being queried, shall be equal to FQDN
        of the node
...
* Upload serialized deployment facts for a node by ID and data source
    ...
    * ``<:endpoint_uri>/environment/<:env_id>/node/<:node_id>/resource/<:datasource>/values``
...
* Upload override of serialized deployment facts for a node by ID and
  data source
    ...
    * ``<:endpoint_uri>/environment/<:env_id>/node/<:node_id>/resource/<:datasource>/override``
...
* **Use Case #3**
  ...
  #. Run the command (change environment and node's IDs to appropriate for
     your environment, change key name to the desired) with ``--override``
     option:

     ::

        fuel config set --env 1 --level node=node-1.domain.tld
            --resource astute --key deployment_id --value 2 --override
