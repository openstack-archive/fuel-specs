...
Cloud operator should be add `cgroups` section into downloaded cluster's settings file to override
...
Cloud operator should be added 'cgroup' section into cluster's settings file via CLI, data
...
User will be able to configure cgroups for set of services using:
    ...
    * CLI - download, introduce `cgroups` sectioni and upload cluster's settings via
