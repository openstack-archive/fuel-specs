...
#. Network interfaces information, that was accessible via the node's model
   ...
   be added, such as discovering port bus ID and vendor driver's information.
...
1. Adding bus and driver's info to nailgun agent, in order to
...
1. Adding bus and driver's info to nailgun NIC db and model.
#. Publish interface's discovered driver and bus info data (under
   interfaces).
...
* Change bin/agent to call driver's functions (int.driver and int.bus_info),
...
2. Publish interface's discovered driver and bus info data:

* Add to the networking dict of deployment serializers the driver and bus ID
  of the node's interfaces, under the interfaces dict.
...
* Unit tests will be added for the new published parameters in Fuel-web.
