...
Is a two ways for use it:

#. Use comand-line options for setting directly. This method stateles and
   puppet don't provide statefull interface for this.

    .. code-block:: puppet

      ethtool => {
        K => ['gso off', 'gro off'],
        s => ['duplex half', 'mdix off']
      }

    will be transformed, to chain of calls:

    .. code-block::

      ethtool -K  %%interface_name%%  gso off
      ethtool -K  %%interface_name%%  gro off
      ethtool -s  %%interface_name%%  duplex half
      ethtool -s  %%interface_name%%  mdix off

#. Use pre-defined stateful sections.
    ...
    You can manage only pre-implemented options, listed here.
...
.. [3] `Ethtool notes <http://www.linuxjournal.com/content/fun-ethtool>.`_
