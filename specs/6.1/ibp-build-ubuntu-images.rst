...
The script accepts the following environmental parameters:
* SEPARATE_FS_IMAGES
  What images will be built and what file system will be used.
  eg: SEPARATE_FS_IMAGES="/boot,ext2 /,ext4"
* UBUNTU_ARCH
  What machine architecture the image will be built for.
  eg: UBUNTU_ARCH=amd64
* UBUNTU_RELEASE
  Ubuntu release codename.
  eg: UBUNTU_RELEASE=precise
* TMP_BUILD_DIR
  Temporary directory to be used for the build.
* DEST_BUILD_DIR
  The directory where the final images with metadata will be stored.
* UBUNTU_KERNEL_FLAVOR
  What kernel flavor will be used.
  eg: UBUNTU_KERNEL_FLAVOR=lts-trusty
* INSTALL_PACKAGES
