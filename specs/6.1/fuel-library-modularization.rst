...
* **parameters** The actual payload of the task. For the Puppet task they can
  ...
  for the task to complete before the tasks if marked failed by time out.
...
Nailgun. Deploy task is a achor used to start the graph traversal and is hidden
from the image.
...
in the groups field of each tasks.
...
require hiera function to work. Obviously, pache should be configured and site
...
* **hiera** Together with modularisation we are moving from the *fuel_settings*
...
*astute.yaml* uploaded by Astute as the first Hiera data source. Aster this
task all the variable found in the astute yaml becomes available as for hiera
...
* **netconfig** This is the first granular task implemented and is just
  ...
  globals task.
* **fierewall** Configures firewall on the node.
...
* Step #1:
  ...
  legacy site.pp manifest). So it's mostly like previous monolithic deployment
  scheme where everything was deployed with a single puppet apply run.
* Step #2:
  Move top-scope roles (primary-controller, controller, compute, cinder,
  ceph-osd) into separate tasks and get rid of 'legacy.pp'.
* Step #3:
  Split top-scope roles into smaller tasks. For example, controller should
  be split into: cluster, haproxy, galera, rabbitmq, openstack::controller,
  etc.
* Step #4:
