...
This specification introduces a clear separation between a base OS (an Red Hat
or Debian based Linux distribution), and an OpenStack distribution (MOS). All
software packages deployed by Fuel are divided into following categories based
on their relation to the specific base OS and OpenStack distribution being
deployed:
...
the MOS dependencies as possible are satisfied by upstream packages from the
base OS.
...
in the same way as a base OS mirror (Red Hat or CentOS).
...
be placed into special mirror which can be specified in Nailgun to deliver
these packages to an environment. Later, this functionality should be exposed
to the community allowing any community engineer (e.g. nova, cinder) to specify
their own git refspec (repository and commit). The build system should be able
to build packages and provide a link which can be passed through Nailgun.
...
For CentOS, there's no special meaning of priorities. Unlike APT, in Yum the
...
followed by a MOS build version. The upstream version of a package should
exactly match the software version, without any suffixes.
...
In upgraded packages, MOS build version should be prefixed with "0~" so that it
is evaluated lower than any possible debian revision assigned to the package
when the same software version appears in the base OS. For example, if base OS
contains version 1.2.3 and version 1.2.4 is required in MOS 6.1, MOS package
version will be 1.2.4-0~0mos1, 1.2.4-0~0mos2 when introduced before GA, and
1.2.4-0~0mos1+mos6.1+1, 1.2.3-0~0mos0+mos6.1+2 when introduced in updates
channel or a maintenance release.

Regressed packages must use the same MOS build version format as in upgraded
packages. For example, if base OS package version 1.2.3-0ubuntu1 introduces a
regression in MOS 6.1, the replacement package versions will be 1.2.3-0~0mos1,
1.2.3-0~0mos2 when introduced before GA, or 1.2.3-0~0mos0+mos6.1+1,
1.2.3-0~0mos0+mos6.1+2 when introduced in updates channel or a maintenance
release.
