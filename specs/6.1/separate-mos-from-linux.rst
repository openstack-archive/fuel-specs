...
presumably Debian style version.  MOS_VERSION is the target MOS release. Adding
packages without such a version suffix is strictly forbidden.

Example: suppose the package foo version 1.2.3-0ubuntu13.10 should be added to
MOS 6.1. The suffix ~mos6.1 should be added to the version, thus the version of
the backported foo package is 1.2.3-0ubuntu13.10~mos6.1

Rationale:
- We need to track the modifications both for technical and for legal reasons.
  Adding version suffix makes such tracking very trivial.

- Switching back to the Ubuntu version of a package should be as easy as
  possible. Adding ~something suffix to the version makes our package formally
  older than the original (this is a common practice of backporting).

- When updating the backported package (such as applying a custom patch) an extra +$
  {PKG_REVISION} suffix.Example: suppose the package foo version 1.2.3-0
  ubuntu13.10~mos6.1 needsa bugfix (which is not available in Ubuntu). A
  fter adding a patchthe version should be changed to 1.2.3-0ubuntu13.10
  ~mos6.1+1Rationale: we need to identify the patched packages without h
  aving tolook at the actual source.3. The only permitted modification 

  of version is adding the above mentionedsuffixes. In particular increm
  enting the original version or truncatingit is strictly forbiddenExam

  ples:OK: 1.2.2-0ubuntu13.1 -> 1.2.2-0ubuntu13.1~mos6.1+1WRONG: 1.2.2-

  0ubuntu13.1 -> 1.2.2-0ubuntu13.2WRONG: 1.2.2-0ubuntu13.1 -> 1.2.2-ubun
  tu1Rationale: make it possible to backport newer revisions (which migh
  tcontain new bugfixes) from Ubuntu without introducing version conflic
  ts.
...
Mandatory Design Reviewers:
  ...
  Tomasz Napirala <tnapirala@mirantis.com>
