...
According to [1]_, [2]_ orchestrator input data will be changed significantly.
...
features in Nova-Network enabled configurations. See [3]_.
...
Parameters to be set in UI on the first stage. See [1]_ ,[4]_ for details.
...
orchestrator uses different notation (see [1]_). Variables ended with '__' will
...
available for certain modes only (see [1]_).
...
This task can be postponed to the next stage (to MOS 7.0).


Alternatives
------------
Task 6 can be done separately. Other tasks should be done all together if
current library changes [1]_ will be done completely.
...
for Cloud Operators [1]_. It should also explain UI flow changes.
