...
Each suite should be a symlink to particular snapshot
  ...
  mos-repos/ubuntu/snapshots/{suite}-{datetime}
...
`snapshots/{suite}-{datetime}` doen't match with `Suite` tag at Release file

So need a way to access snapshot with origin suite name.
We can create supporting folder structure:
