...
* As a CI engineer, I need a package build system that doesn't introduce
  package management limitations compared to upstream Linux distros used in
  MOS
...
* Code storage
  ...
  Gerrit projects structure:
    ...
    - MOS linux packages
      code+spec projects: `[customer-name]/packages/{distro}/{package name}`
    - Master-node linux packages (separated from MOS linux in 7.0)
      code+spec projects: `[customer-name]/packages/fuel/{distro}/{package name}`
...
* Build workers
  Hardware nodes with preconfigured build tools for all supported distros.
  Will be configured as Jenkins slave.
  Will contain:
    - preconfigured docker images with native build tools for each distro type:
      mockbuild: will build packages by mock (`centos6/7` target distros)
      sbuild: will build packages by sbuild (`trusty` target distro)
    - prepared minimal build chroots for all supported distros
...
* Publisher
  ...
  Will be configured as Jenkins slave.
  Pepositories will be maintained by native tools of respective distro
...
* Open Build Service (OBS) (see `Problem description`):
  Supports rpm/debian based distros
  http://openbuildservice.org/

* Koji:
  Supports rpm based distros only
