...
container which provides stevedore endpoints for network verificatopn
service. Fuel plugin also publish some executable entities (shell scripts, python stuff, etc) via web server (like it does now for plugin’s Puppet modules).
