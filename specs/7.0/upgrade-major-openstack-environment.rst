...
Compute and/or Storage roles. Resource nodes are upgraded 'in place', i.e. on
the same hardware, keeping user data intact on storage devices separated from
Operating System boot device on the node. Operating system and all software on
the node is reinstalled by Fuel installer.

The reason to have external script that performs operations outlined above is
that it have to orchestrate at least 2 OpenStack environments: the original
...
* Control plane services on Compute nodes in 5.1.1 environment are upgraded
  to 6.0 without affecting virtual server instances and workloads. After
  upgrade, Compute service reconnects to 6.0 CICs with the same version of
  RPC.
* Compute nodes from 5.1.1 environment work with CICs from 6.0 environment,
  ...
  Compute nodes one by one by re-assigning to 6.0 environment and
...
* Compute/Storage nodes will be moved from original version 5.1.1 environment
