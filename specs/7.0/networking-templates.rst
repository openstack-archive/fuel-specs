...
one will use DB to fetch info about networks and IPs as it is synchronized.

IPs allocation for nodes in case of template will take in account what networks
are in use on particular node.
...
* Nailgun:
   ...
   h. IPs allocation using info about network to nodes mapping
      (Estimate: 2d)
   i. Add section [network_roles] into task descriptions
      (Estimate: 1-2d + library to provide info)
   j. Provide data for network verification tool in case of template
      (to be estimated)
