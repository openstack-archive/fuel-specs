...
untrusted, the encrytion is required to ensure confidentiality.
...
services use the public endpoint. For these reason we need to explicitly
configure all OpenStack services to use the internal API endpoint when
communicating with other services. If a project doesn't offer this
possibility we will then configure the service with the correct CA
for authentication.
...
- When an OpenStack service queries another OpenStack service it must use the
  ...
  authority's certificate used by the HAProxy.
...
- In case of "real" HA where several HAProxy are deployed, we need to manage
