...
cloud-init is able to read. Configdrive is supposed to be put in the end
of one of hard drives on a node during provisioning stage.

Astute provision method won't add node records into cobbler. This method should
be re-written so as to run provision script on nodes that are supposed to be
provisioned.
...
into configuration stage not provisioning. Ironic is not going to support
OS agent based power management (only IPMI, ILO, DRAC, etc.) That is why it is
better to solve those issues Fuel currently has that are related to
...
it is going to become around 500M bigger.
