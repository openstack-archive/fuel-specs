...
We need to prepare test scenarios, to check if plugins can be upgraded
successfully.
- System tests should be created to verify installing multiple plugin versions
  on Fuel Master node
- System tests should be created to verify upgrading of a major plugin verion
  on environments
...
* There is a possibility to apply a major version to a live plugin on an existing environment
