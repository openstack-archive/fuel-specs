...
* Supply file ``astute.yaml`` from backup to the freshly installed Fuel
  Admin node via ``fuel-upgrade restore`` command. It will restore
  credentials from the supplied ``/etc/fuel/astute.yaml`` file.

* Command ``fuel-upgrade restore`` shall inject database dump into
  PostgreSQL DB and run migration scripts on the database.

* Command ``fuel-upgrade restore`` shall upload ``openstack.yaml`` fixture
...
* use ``fuel-upgrade`` to extract the backup file and restore
...
* ``fuel-upgrade restore`` restores contents of ``/etc/fuel/astute.yaml``
  file in the following sections from values in backup ``astute.yaml`` file:
