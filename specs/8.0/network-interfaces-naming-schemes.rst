...
naming. Such a naming is known to be unstable in the following ways::
  ...
  2) the interfaces' names are not preserved across the reboots.
     The 2nd problem can be solved by udev rule which pins the interface name
     to the MAC address of the NIC. Solving the 1st problem requires a
     different naming convention, such as the predictable network interfaces'
     names (as implemented in udev >= 197). Modern Linux distributions
     (Ubuntu >= 15.04, CentOS >= 7) use this scheme by default, and it can be
     enabled in Ubuntu 14.04 (and Debian Jessie).

Hard-coded interface names ethX in Fuel code should be changed to support
arbitrary interface naming schemes.
Currently, the conventional interface naming policy is implemented.
It doesn't enable, for example, switching to predictable interface naming
policy in Ubuntu or switching to systemd used in contemporary Linux
...
upgrading of the Fuel master node, so it is necessary to describe whether
this patch set contradicts upgrade process itself or any supported working
feature.
