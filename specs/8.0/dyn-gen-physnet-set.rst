...
configuration classes. Parameter 'flags' is needed for adding an ability to
disable some physnets. By default 'flags' field looks like::
...
Example of usage: Neutron config on compute node, if DVR is chosen,
should contain external (floating) physnet in the mappings. However if DVR is
not chosen, this physnet shouldn't be present in config file.
...
Plugin developers got ability to override standart physnet list.
...
* patch ml2-plugin, l3-agent and network creating granulas
  (puppet manifests) in osnailyfacter module
* modify NOOP-tests
...
Noop-tests should be written additionally for cases
where physnet list should be extendedю


Acceptance criteria
===================

Acceptance criteria is the same as for fuel-library ordinary bug-fixes.
Standard BVT should be passed for ordinary configurations
