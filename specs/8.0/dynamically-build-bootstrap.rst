...
.. code-block:: bash
  ...
  --ubuntu-repo 'http://archive.ubuntu.com/ubuntu trysty main universe 
    multiverse restricted'

--mos-repository REPOSITORY     Add link to repository with fuel* packages.
                                Tt should be either http://mirror.fuel-infra.org/mos-repos
                                or its mirror.
  **Warning:** mos-repository is mandatory variable.
--repository REPOSITORY         Add additional repository

REPOSITORY format:
    The '--repository' option can be specified multiple times, several 
    repositories will be added.
    --repository 'uri distribution [component],[priority]'
    --repository 'http://mirror.fuel-infra.org/mos-repos/ubuntu/8.0 mos8.0 
      main,priority=1101'
    --repository 'http://mirror.fuel-infra.org/mos-repos/ubuntu-test/9.0 
      mos9.0 main,priority=1120'
    Apt_pinning_docs_.
.. _Apt_pinning_docs: https://www.debian.org/doc/manuals/debian-reference/ch0
   2.en.html#_tweaking_candidate_version
...
--package PKGNAME               Install extra package from the repository.
                                Variable are iterable - so you can pass
                                multiply packages.
...
--inject-files-from DIR_OR_ARCHIVE   Directory or archive that will be injected
  ...
  **Note** Files/packages will be injected after installing all mandatory 
           packages, but before generating system initramfs - so you can add 
           something specific, like udev rules or kernel modules. Same for 
           --script parameter.
