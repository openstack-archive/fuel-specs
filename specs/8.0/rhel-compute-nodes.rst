...
An Minimal Viable Product is to provide detailed documentation for enabling
...
* How to add MOS repositories on this node;
...
this feature in 9.0 - automate deployment and support RHEL computes at scale.
...
* Allow Fuel to deploy environments with multiply operating systems
...
* User creates base MOS cloud (Fuel Master + Controllers) using standard Fuel
  ...
  * Neutron+OVS (VXLAN) on networking layer
...
* Mirantis QA has the way to test updates in code of MOS packages for RHEL

* Cloud deployment engineer can prompt RHEL node to install latest versions
  of MOS packages ("yum update" or manually)
