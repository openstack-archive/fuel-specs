...
Feature is targeted to Fuel 8.0 release, so each environment which was
...
This feature require separate hardware server where we will run new
SWARM & BVT tests. We also need to prepare qcow2 image for compute nodes
and upload this image to all hardware servers where we will execute
new SWARM tests.

We need configure CI jobs per commit for all changes in RPM packages
