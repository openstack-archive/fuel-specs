...
"security-<id>" where ID is the timestamp of update bundle. These branches
must be based on the particular commits that correspond to the previously
released version of a package. In case if there was no security updates for
a given package, an appropriate security branch should point to the
