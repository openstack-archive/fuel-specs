...
the appropriate directory using puppet's facilities:
...
puppet manifest (which one?). Additionally, the 'revoke_by_id' parameter will
be set to 'false' as it is not required for Fernet tokens and, if set to 'true'
