...
defently compatible components with 'green light'. Logic to process very
similar to 'requires'. We should conjunction element of different types and
disjunction of one type. For example:
...
In this case storage A compatible with hypervisor A OR hypervisor B AND
network A. And storage B compatible with  hypervisor A OR B and any type
of network. On Compute tab we can describe compatibilities between
hypervisors, on Network between networks and hypervisors, on Storage between
storges storages, networks and hypervisors, on Additional services between
all.
