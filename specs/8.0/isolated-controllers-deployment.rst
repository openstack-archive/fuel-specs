...
Proposed logic shall be implemented in class ``UpgradeSerializer`` in module
``orchestrator.upgrade_serializers``.
...
* Implement handler ``UpgradeNodeHandler`` and task ``UpgradeNodeTask`` to
  update status of the node, create new deployment info for it and cast RPC
  request for deployment to Astute.

* Implement cluster and node serialization logic as the corresponding
  classes in ``orchestration.upgrade_serializers`` module.

* Implement network configuration serialization logic to exclude patch ports
  for isolated deployment mode.
