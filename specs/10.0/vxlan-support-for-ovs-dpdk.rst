...
described in Data model section. Also we should assign the IP allocated from
`private` network on `br-mesh` bridge.
...
* Fix network serializer so that transformations were configured as described
  in Data model section, additionally we should assign IP allocated from
  `private` network on `br-mesh` bridge
* Test manually
