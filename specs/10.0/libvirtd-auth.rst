...
cluster - in this case a libvirtd daemon running on the source compute host
...
system libraries for that. Another point is that only libvirt management
traffic is encrypted, not the actual data of VMs being live migrated.
...
It's worth to issue a separate technical bulletin on how to enable libvirtd
authentication for already deployed clusters, as well as for previous MOS
releases.
...
libvirtd didn't break anything.

Additional test is needed to make sure connecting to a libvirtd daemon on a
...
* live migration of VMs between compute nodes is still working
