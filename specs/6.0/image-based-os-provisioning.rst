...
We are planning to add provision metadata json field into release nailgun
database model. Those data are supposed to contain information about which
provision method to use for this relaese (cobbler based or image based) and
information about available OS images which are assumed to be built and put on
ISO. So user will be able to choose between two alternative provision methods.
